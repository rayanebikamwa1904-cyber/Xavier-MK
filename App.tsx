...
// PASSAGE EN MINUSCULES POUR COMPATIBILITÉ NETLIFY
...
const App: React.FC = () => {
            ...
            const sections = [
                ...
                },
                {
                    ...
                    content: {
                        ...
                        stats: { years: data.yearsOfExperience || 1, projects: data.reviewCount || 0 }
                    }
                },
                { type: 'services', content: { items: data.catalog || [] } },
                { type: 'gallery', content: { images: data.projects?.map((p: any) => p.image).filter(Boolean) || [] } },
                {
                    type: 'contact',
                    content: { address: "Kinshasa, RD Congo", actionValue: data.whatsapp, phone: data.phone, email: data.email }
                },
                { type: 'social', content: { userAvatar: data.profileImage, posts: data.posts || [] } }
            ];

            let sortTimestamp = new Date().toISOString();
            if (data.activatedAt) {
                sortTimestamp = typeof data.activatedAt.toDate === 'function' ? data.activatedAt.toDate().toISOString() : String(data.activatedAt);
    ...
    <div className="min-h-screen bg-majestic-gradient text-white font-sans">
      ...
      <section className="relative pt-40 pb-20 px-4 text-center">
        ...
        <p className="text-xl text-gray-400 max-w-2xl mx-auto mb-10">
          Obtenez votre site pro en 2 minutes et rejoignez le réseau mondial des prestataires.
        </p>
      ...
      <section className="max-w-7xl mx-auto px-4 py-20 text-center">
        <h2 className="text-3xl font-bold mb-10">Les Stars de l'Arène <span className="text-gold-400">✦</span></h2>
        {creators.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                {creators.slice(0, 3).map((creator) => (
                  <GlassCard key={creator.id} onClick={() => { setSelectedCreator(creator); setView(AppView.ARENA); }} className="p-0 hover:scale-[1.02] transition cursor-pointer group">
  ...
  return <div className="min-h-screen bg-black flex items-center justify-center text-gold-400">Chargement de l'Empire...</div>;
};
export default App;
