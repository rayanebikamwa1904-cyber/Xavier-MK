      ...
      const fetchCreatorBySlug = async () => {
        ...
        if (!querySnapshot.empty) {
          ...
          const sections = [
              ...
              { type: 'bio', content: { sectionTitle: finalData.bioSectionTitle || (finalData.userType === 'enterprise' ? 'Qui sommes-nous ?' : 'À Propos'), image: finalData.profileImage, name: finalData.name, description: finalData.bio, stats: { years: finalData.yearsOfExperience || 1, projects: finalData.reviewCount || 0 } } },
              { type: 'services', content: { sectionTitle: finalData.skillsSectionTitle || 'Mes Compétences', items: finalData.catalog || [] } },
              { type: 'gallery', content: { sectionTitle: finalData.projectsSectionTitle || 'Mes Réalisations', images: finalData.projects?.map((p: any) => p.image).filter(Boolean) || [] } },
              { type: 'contact', content: { sectionTitle: finalData.contactSectionTitle || 'Contact', address: "Kinshasa, Gombe", actionValue: finalData.whatsapp, phone: finalData.phone, email: finalData.email } },
              { type: 'social', content: { sectionTitle: finalData.socialSectionTitle || 'Mon Actualité', userAvatar: finalData.profileImage, posts: finalData.posts || [] } },
              { type: 'catalog', content: { sectionTitle: finalData.catalogSectionTitle || 'Catalogue' } },
              { type: 'testimonials', content: { sectionTitle: finalData.testimonialsSectionTitle || 'Ils ont adoré' } },
              { type: 'partners', content: { sectionTitle: finalData.partnersSectionTitle || 'Ils nous font confiance' } },
              { type: 'experience', content: { sectionTitle: finalData.experienceSectionTitle || 'Mon Parcours' } }
          ];
    ...
    case AppView.ARENA:
      if (selectedCreator) {
        return (
          <div className="min-h-screen bg-black relative">
            <SEO 
              title={`${selectedCreator.name} | Portfolio Officiel`}
              description={`Découvrez l'univers de ${selectedCreator.name}, expert en ${selectedCreator.category}. Visitez son Empire numérique sur My Folio-Tag.`}
              image={selectedCreator.portfolio.sections[0].content.backgroundImage || selectedCreator.portfolio.sections[1].content.image}
              type="profile"
            />
            <ErrorBoundary>
              <PortfolioPreview 
